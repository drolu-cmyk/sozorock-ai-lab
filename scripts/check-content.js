...
const requiredHomepageCopy = [
  ...
  'Learning in practice', 'Edward Jones', 'Capital Property Care',
  'Human judgment stays in control throughout.',
  'Participant story and media shared with consent.',
  'Responsible AI skills', 'AI workshops and guided learning',
  'Evidence, not hype', 'For community organizations',
...
for (const text of requiredOrganizationCopy) if (!organizations.includes(text)) failures.push(`Organizations page missing approved partner copy: ${text}`);

const forbiddenPublicPatterns = [
  /Before we automate/i,/AI Capability Lab/i,/Day\s+\d+\s+of\s+\d+/i,/context engineering/i,/context pack/i,/evidence saved/i,/program outcome/i,/capstone defense/i,/\bGRC\b/i,/SozoRock Meridian|Meridian/i,
...
if (/>(?:\s*)0[1-9](?:\s*)</.test(html) || />(?:\s*)0[1-9](?:\s*)</.test(organizations)) failures.push('Primary campaign pages contain numbered visual UI; approved direction is unnumbered.');
...
if (!organizations.includes('<details class="form-more">')) failures.push('Organization form optional planning fields are not progressively disclosed.');
...
for (const source of [html, organizations]) if (!source.includes('Do not submit private customer, patient, student, employee, financial, legal, or account information.')) failures.push('Sensitive-information warning is missing from a primary form.');
...
if (!html.includes('/assets/css/option-3.css?v=20260808-1') || !html.includes('/assets/css/campaign.css?v=20260808-3') || !html.includes('/assets/js/main.js?v=20260808-1')) failures.push('Current homepage asset versions are missing.');
if (!organizations.includes('/assets/css/campaign.css?v=20260808-3') || !organizations.includes('/assets/js/main.js?v=20260808-1')) failures.push('Current organization-page asset versions are missing.');
...
console.log(`Content checks passed: campaign positioning, authentic participant evidence, partner/funder pathway, unnumbered visual structure, curriculum, organization inquiry, ${legalPages.length} policy pages, forms, CTA destinations, and internal links.`);
